...
def test_rt5572_firmware_exists():
    base_dir = Path(__file__).parent.parent
    fw_path = base_dir / "src" / "wifit3" / "chips" / "rt5572" / "assets" / "rt5572.bin"
    
    assert fw_path.exists(), f"RT5572 Firmware missing at {fw_path}"
    assert fw_path.stat().st_size > 1000, "RT5572 Firmware file is too small to be valid."
