"""RX reader-thread path (port of rtl8821au commit 2e3a7a7): the dedicated
thread must keep reading and hand buffers to the loop, which parses + fires the
callback — without that thread blocking on the event loop. Validates the
thread->loop->callback flow and a clean shutdown (thread joins, no leak)."""
import asyncio
...
from wifit3.chips.rtl8822bu import driver as drv
