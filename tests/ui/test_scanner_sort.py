...
async def test_sort_wps_hierarchy_and_power_tie_break():
    ...
    async with app.run_test() as pilot:
        app.array = _FakeArray([ap_unlocked_weak, ap_unlocked_strong, ap_locked, ap_no_wps], [1, 6, 11])
...
async def test_immediate_sort_on_new_ap_arrival():
    ...
    app = WifiteApp()
    async with app.run_test() as pilot:
        app.array = _FakeArray([ap_mid, ap_weak], [1, 6, 11])
        app.push_screen("scanner")
        await pilot.pause(0)
        scanner = app.screen
        table = scanner.query_one("#ap-table", DataTable)
...
async def test_sort_aps_short_circuits_when_order_unchanged():
    ...
    app = WifiteApp()
    async with app.run_test() as pilot:
        app.array = _FakeArray([ap1, ap2], [1, 6, 11])
        app.push_screen("scanner")
        await pilot.pause(0)
        scanner = app.screen
        table = scanner.query_one("#ap-table", DataTable)
...
async def test_sort_channel_ascending_breaks_ties_with_stronger_power():
    ...
    app = WifiteApp()
    async with app.run_test() as pilot:
        app.array = _FakeArray([ap_ch6_weak, ap_ch1, ap_ch6_strong], [1, 6, 11])
        app.push_screen("scanner")
        await pilot.pause(0)
        scanner = app.screen
        table = scanner.query_one("#ap-table", DataTable)
...
async def test_sort_ssid_ascending_sinks_hidden_networks():
    ...
    app = WifiteApp()
    async with app.run_test() as pilot:
        app.array = _FakeArray([ap_b, ap_hidden, ap_a], [1, 6, 11])
        app.push_screen("scanner")
        await pilot.pause(0)
        scanner = app.screen
        table = scanner.query_one("#ap-table", DataTable)
