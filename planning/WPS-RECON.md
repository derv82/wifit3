...
Scope: `src/wifit3/engine/attacks/{auth_assoc.py, campaign.py, wps/*}`, `wlan/interface.py`,
...
- **`Campaign` (base)** — `engine/attacks/campaign.py:27`. Radio-owning attack base; class-level
...
- **`WpsCampaign(Campaign)`** — `campaign.py:97`. Focus-facing PIN brute-force orchestrator:
  ...
  (`:123-176`).
- **`WpsPbcCapture(Campaign)`** — `wps/pbc.py:47`. Fire-once opportunistic PBC PSK capture;
  associates as Enrollee and runs `WpsEnrollee`.
- **`WpsRegistrar`** — `wps/registrar.py:131`. Transport-agnostic per-PIN EAP/WSC state machine
...
- **`WpsEnrollee`** — `wps/enrollee.py:32`. Mirror-polarity WSC machine for PBC (we build
  M1/M3/M5/M7, PSK arrives in M8). Same transport contract as the registrar.
- **`WpsTransport` (Protocol)** — `wps/registrar.py:34`. A structural `typing.Protocol`
  (`send_until_ack`/`send_no_wait`/`recv`). NOT a real base class; `WlanTransport` satisfies it by
  shape only.
- **`Association`** — `auth_assoc.py:120`. Open-System auth + Assoc Request against one AP;
  appends caller-supplied `assoc_trailer_ies` (the WPS vendor IE).
- **`WlanTransport`** — `auth_assoc.py:58`. Adapts a `WlanInterface` to the send/recv/drain
...
`Association` (`campaign.py:264`) + `WlanTransport` (`:269`); `._try` builds `WpsRegistrar` (`:282`).
...
```mermaid
graph TD
    A1["WpsCampaign._try<br/>campaign.py:275"]
    A2["WpsCampaign._ensure_session<br/>campaign.py:255"]
    P1["WpsPbcCapture.capture<br/>pbc.py:74"]
    R1["WpsRegistrar.try_pin<br/>registrar.py:181"]
    R2["WpsRegistrar._send_1x<br/>registrar.py:171"]
    E1["WpsEnrollee.run<br/>enrollee.py:73"]
    E2["WpsEnrollee._send_1x<br/>enrollee.py:63"]
    AS1["Association.associate<br/>auth_assoc.py:182"]
    AS2["Association._send_until<br/>auth_assoc.py:214"]
    T1["WlanTransport.send_until_ack<br/>auth_assoc.py:94"]
    T2["WlanTransport.send_no_wait<br/>auth_assoc.py:99"]
...
(`campaign.py:418`) → `driver.enable_rx_acks`; `iface.set_fake_mac` (`campaign.py:261`, `pbc.py:82`)
→ `driver.enter_active_monitor`; `iface.acks_seen` (`campaign.py:463`) → `driver.acks_seen`.
...
|---|---|
| **our_mac** | `WpsCampaign.our_mac` `campaign.py:129`; `Association.our_mac` `auth_assoc.py:133`; `WlanTransport.our_mac` `:63`; `WpsRegistrar.our_mac` `registrar.py:151`; `WpsEnrollee.our_mac` `enrollee.py:49`; `WpsPbcCapture.our_mac` `pbc.py:58`. Plus `Driver._our_tx_macs` `driver.py:80` and `WlanInterface.forged_macs`/`self_macs` `interface.py:151,155`. Threaded into 6 objects; rebuilt each re-assoc. |
| **bssid** | str in `WpsCampaign`/`Association`/`CampaignState`; bytes in `WlanTransport`/`WpsRegistrar`/`WpsEnrollee`; `WpsCampaign` re-derives `str_to_mac(self.bssid)` inline at `campaign.py:261,263,269,282`. |
...
| **lock state** | `LockTracker.strikes`/`locked_since` `lock.py:31-33` (real); `WpsCampaign._lock_kind`/`_lock_end_at`/`_consecutive_locks_no_progress` `campaign.py:154-156` (UI mirror); `AccessPoint.wps_locked` (beacon). Three sources feed one decision at `campaign.py:439`. |
| **last-sent frame** | `WpsRegistrar._last_1x_frame` `registrar.py:169`; `WpsEnrollee._last_1x_frame` `enrollee.py:58`. |
...
- **Nested `def` inside async method inside class:** `WpsRegistrar.try_pin` → `def _out(...)`
  `registrar.py:204`; `WpsEnrollee.run` → `def once(...)` `enrollee.py:91` + `def phase(...)` `:100`.
...
- **Protocol that should be a class:** `WpsTransport` `registrar.py:34` vs `WlanTransport`
  `auth_assoc.py:58`; the two `_send_1x` impls (`registrar.py:171`, `enrollee.py:63`) are
  near-identical duplicates.
- **Constructor-injected deps re-threaded every attempt:** each per-PIN attempt reconstructs
  `Association` + `WlanTransport` + `WpsRegistrar` (`campaign.py:264-282`), re-wiring the same
...
to `WlanTransport` and reaches straight to `WlanInterface.send_no_wait` (`auth_assoc.py:222`); (2)
...
leaving-deauth with no protocol object between it and the interface (`pbc.py:120`); (3) identity
