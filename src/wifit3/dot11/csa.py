...
def build_csa_beacon(beacon: bytes, new_channel: int) -> bytes:
    """The AP's beacon, sequence-control zeroed (the injector re-stamps each frame), with any
    existing CSA element replaced by a fresh immediate-switch CSA to ``new_channel``. Injectable MPDU."""
    ...
    while ptr + 2 <= len(tags):
        ...
        if tags[ptr] != _ELEMID_CSA:
            kept += tags[ptr:end]
        ptr = end
    return bytes(header) + bytes(kept) + csa_ie(new_channel)
