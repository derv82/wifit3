"""RTL8821A JaguarSeries phy_cond table walker (BB / AGC / RF init tables).
...
ports the 8821a 4-condition walker + check_positive 1:1 from
`halhwimg8821a_bb.c:21-106` (check_positive) and `:366-415` (the walker loop,
identical across the three tables). `apply_table` emits write_fn(addr, value) for
each taken data row.
...
@dataclass
class JaguarParams:
    """Condition inputs for check_positive. Defaults are the AWUS036ACS values."""
    cut_version: int = 0
