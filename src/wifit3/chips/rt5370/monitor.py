"""Monitor-mode entry for the RT5370 (RT5390) — the ``airmon-ng start`` register sequence.
...
def config_retry_limit(t: RT5370Transport, short_retry: int = DEFAULT_SHORT_RETRY,
...
def config_ps(t: RT5370Transport) -> None:
...
def config_erp(t: RT5370Transport, *, short_preamble: bool, cts_protection: bool,
...
def initial_config(t: RT5370Transport, chip: ChipInfo, ev: EepromValues,
...
def enable_monitor(t: RT5370Transport, chip: ChipInfo, ev: EepromValues, drv=None,
                   short_retry: int = DEFAULT_SHORT_RETRY) -> None:
    ...
    (RT5390 has no init-derived calibration). ``short_retry`` routes to TX_RTY_CFG
    SHORT_RTY_LIMIT (default 7)."""
