...
class RT5370Transport:
    ...
    def _vendor_request(self, requesttype, request, value, index, data_or_length):
        ...
        for attempt in range(_VENDOR_REQ_MAX_ATTEMPTS):
            try:
                ...
                if attempt:
                    logger.info("rt5370: vendor req 0x%02x off 0x%04x recovered on attempt %d",
        ...
        logger.warning("rt5370: vendor req 0x%02x off 0x%04x failed after %d attempts: %s",
    ...
    def regbusy_read(self, offset: int, field_mask: int,
        ...
        for _ in range(count):
            raw = self.register_multiread(offset, 4)
            # A read that returns no value can't prove the busy bit cleared, so keep
            # polling — the kernel only exits on a positive busy=0 read. (One RF_CSR_CFG
            # busy poll in the cold-boot capture records an empty completion for its
            # first, still-busy iteration; collapsing that to 0 would falsely read
            # "not busy" and skip the second poll the kernel did.)
            if len(raw) < 4:
                continue
            reg = int.from_bytes(raw, "little")
    ...
    def eeprom_read(self, length: int) -> bytes:
        ...
        #TODO untestable: this card is an EFUSE card, so the chip never takes
