"""RTL8188FTV USB transport — vendor-control register reads/writes.

Mirrors `rtl8xxxu_read8/16/32` and `rtl8xxxu_write8/16/32` from
`driver_sources/rtl8xxxu-source-v6.12.107/core.c` (lines 599-825). All MAC
...
8188f (rtl8xxxu) family stays self-contained.
"""
...
class RTL8188FTVTransport:
    """Vendor control-transfer transport for the RTL8188FTV."""
    ...
    # ---- RF register access (core.c:915-942) --------------------------
    # RFREG traffic is a plain 32-bit LSSI write to REG_FPGA0_XA_LSSI_PARM
    # with the RF addr in the top bits.  Reads use the same LSSI register.
    def read_rfreg(self, path: int, reg: int) -> int:
        return self.read32(REG_FPGA0_XA_LSSI_PARM + (path * 4))

    def write_rfreg(self, path: int, reg: int, data: int) -> None:
        data &= 0x000FFFFF
        addr = (reg << 20) | data
        self.write32(REG_FPGA0_XA_LSSI_PARM + (path * 4), addr)
