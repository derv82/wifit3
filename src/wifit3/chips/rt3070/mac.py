...
def write_mac_address(t: RT3070Transport, mac: bytes, u2me_mask: int = 0x00) -> None:
    """Program the chip's self-MAC into MAC_ADDR_DW0/1 (0x1008/0x100c).

    rt3070's cold bring-up never sets this, so the MAC-match engine has no identity
    and the autoresponder can't ACK us. ``u2me_mask`` (DW1[23:16]) gates that match:
    0 = match nothing (monitor default — promiscuous capture, no spurious ACK); 0xFF =
    strict-match, so active-monitor HW-ACKs only frames to ``mac``.
    [SRC rt2800.h MAC_ADDR_DW0/1]"""
    if len(mac) != 6:
