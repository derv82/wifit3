"""Monitor-mode entry for the RT3070 — the ``airmon-ng start`` register sequence.
...
The channel tunes that follow (airodump/iw) are ``chan.set_channel`` calls, not part
of entry. ``config_erp`` is ported here for a future managed-mode port but is *not*
called in monitor mode (it is BSS-change driven; a monitor vif has no BSS).
...
# mac80211 hw->conf retry defaults (IEEE80211 short=7 / long=4) [SRC mac80211 conf].
DEFAULT_SHORT_RETRY = 7
DEFAULT_LONG_RETRY = 4


def config_retry_limit(t: RT3070Transport, short_retry: int = DEFAULT_SHORT_RETRY,
...
def config_ps(t: RT3070Transport) -> None:
    """Power-save config, STATE_AWAKE path [SRC rt2800lib.c:5649-5677
    rt2800_config_ps]. Monitor never sleeps: clear AUTOWAKEUP_CFG, then wake the MCU
    (``set_device_state(STATE_AWAKE)``)."""
...
def config_erp(t: RT3070Transport, *, short_preamble: bool, cts_protection: bool,
               basic_rates: int, slot_time: int, eifs: int, beacon_int: int) -> None:
    """ERP / BSS parameters [SRC rt2800lib.c:2177-2222 rt2800_config_erp].

    #TODO untestable here: BSS-change driven (managed/AP mode); a monitor vif has no
    BSS so this is never called in the airmon capture. Ported for a future STA port.
    The HT-opmode arm is omitted (this driver is 20 MHz / no aggregation bring-up)."""
