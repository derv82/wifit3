...
(`scripts/rt5572/verify_pcap.py`) call. The gate's cold phase is a single call to that real
function, so it tests exactly what runs on hardware; there is no second copy to drift.

- **Whole capture green** (39408/39408, PASS). The gate drives the driver's real functions:
  `bring_up()` (1781 cold ops byte-for-byte) + `chan.reconfig_channel()` per hop (126/126 hops
  byte-for-byte, all channels incl. 5 GHz). wifit3's monitor-first deviations are **named + counted
  waivers**, not reproduced: airmon STA→monitor entry (wifit3 writes 0x11), per-hop
  RX-toggle/survey interleaving (wifit3 self-brackets), aireplay TX_STA_FIFO polling.
- **Focus-dead-radio fix (untested on HW):** `Driver.set_channel` now brackets `reconfig_channel`
  with the RX quiesce — pause RX reader + stop_queue(RX) → reconfig → start_queue(RX) + resume.
  The kernel disables RX around config_channel or the RF/BBP writes don't latch. Register reconfig
  is gate-verified; the toggle+reader-pause orchestration needs a **HW soak on the PAU09** against
  the focus-ch1-after-5GHz repro.
- **NOT YET DONE:** (1) HW test on the PAU09 (5 GHz TX + focus); (2) reconcile the live monitor
  entry (connect() writes `RX_FILTER_CFG=0x11` after `bring_up()`; the kernel's airmon dance is
  waived) — decide whether to keep 0x11 or adopt a kernel-faithful monitor entry; (3) strip the
  dead RT3572/RT5392 silicon-dispatch branches in chan/rfcsr/bbp/reg_init (now that the gate covers
  the operational path); (4) register `148f:5572` here + stop rt2800usb claiming it; (5) rewrite the
  inherited rt2800usb notes below.
