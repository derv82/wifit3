...
- **Cold bring-up: byte-for-byte green** (1781 ops) driving the real `bring_up()`. Kernel-faithful
  order (probe → EFUSE-before-firmware → gpio → xtal → firmware → radio-on → init_registers →
  BBP → RFCSR → enable_radio). Dropped `connect()`'s non-kernel extra reads (read_perm_mac,
  is_chip_warm, PBF-READY check).
- **NOT YET DONE:** (1) operational phase in the gate — drive the real `set_channel` + a monitor
  entry at wire openers, **waive** aireplay's TX_STA_FIFO (do not reproduce it); (2) the
  `set_channel` RX-toggle/config_ant/reset_tuner bracket (the suspected focus-dead-radio /
  band-transition bug); (3) HW test on the PAU09 (5 GHz TX + focus); (4) strip the dead
  RT3572/RT5392 branches; (5) register `148f:5572` here + stop rt2800usb claiming it; (6) rename
  `RT2800USBDriver`/`RT2800USBTransport` and rewrite this doc (below is inherited rt2800usb notes).
- The live `connect()` still writes the monitor-first `RX_FILTER_CFG=0x11` after `bring_up()`
  (deliberate deviation, not in the cold gate) — reconcile in the operational milestone.
