"""Channel tuning + TX-power for the RT5372 (RT5392, RF5372, 2T2R, 2.4 GHz).

Ported from ``rt2800lib.c``: ``rt2800_config_channel`` dispatch (4161) →
``rt2800_config_channel_rf53xx`` (3387) for RF5372; the per-tune VCO calibration
...
captures_rt2800usb_rt5372/capture-1 by ``verify_pcap.py rt5372``.
...
Scope: RF53xx 2.4 GHz / 2T2R. RT5392 has NO RX-filter loopback calibration, so the tune
takes no init-derived calibration state (``drv`` is unused / ``None``). 5 GHz and the
foreign-family / 40 MHz arms are out of scope (this driver claims only 148f:5372).
"""
...
def update_survey(t: RT5372Transport) -> None:
...
def config_ant(t: RT5372Transport, chip: ChipInfo, ev: EepromValues) -> None:
    """RX/TX antenna select via BBP1/BBP3 [SRC rt2800lib.c:2322-2398 rt2800_config_ant].
    PAU05/PAU06 are 2T2R (txpath=rxpath=2) → BBP1 TX_ANTENNA=2, BBP3 RX_ANTENNA=1 (the
    non-RT3572-BT arm). The 3T3R arms are #TODO untestable."""
    r1 = t.bbp_read(1)
    r3 = t.bbp_read(3)
    ...
    if ev.rx_chain_num == 1:
        # The SW-diversity sub-branch is RT3070/3090/3352/3390-only — never on RT5392.
...
def freq_cal_mode1(t: RT5372Transport, ev: EepromValues) -> None:
...
def config_channel_rf53xx(t: RT5372Transport, chip: ChipInfo, ev: EepromValues,
    ...
    RFCSR49/50; freq trim via the MCU; per-channel RFCSR59 from the non-BT table."""
...
def config_channel(t: RT5372Transport, chip: ChipInfo, ev: EepromValues,
    ...
    rt2800_config_channel], RF53xx / 2.4 GHz / 2T2R path."""
...
def _config_tx_pin_pa(tx_pin: int, tx_chain_num: int, channel: int) -> int:
    """TX_PIN_CFG PA-enable switch [SRC rt2800lib.c:4363-4388]. The secondary PAs are
    live on this 2T2R card; the tertiary (3T3R) arm is #TODO untestable. No BT-coexist."""
    is_a = int(channel > 14)
    is_g = int(channel <= 14)
    if tx_chain_num >= 3:
...
def config_txpower(t: RT5372Transport, chip: ChipInfo, ev: EepromValues) -> None:
    ...
    RT3070/3071/3090/3572-only — N/A for RT5392, so delta stays 0."""
...
def set_channel(t: RT5372Transport, chip: ChipInfo, ev: EepromValues,
    ...
    ``drv`` is unused (RT5392 threads no init-derived calibration); kept for the family
