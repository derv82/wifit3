"""AGC / VGC link tuning for the RT5372 (RT5392) — BBP66 sensitivity.
...
Two consumers:
  * ``reset_tuner`` — run on every channel config (``rt2x00link_reset_tuner``). The
    kernel zeroes ``qual->vgc_level`` first, so the default VGC is *always* re-written
...
def get_default_vgc(chip: ChipInfo, lna_gain: int, band_2ghz: bool = True) -> int:
    ...
    if band_2ghz:
        # RT5390/RT5392 (and the rt3/rt5 family) use 0x1c + 2*lna_gain; the legacy
        # 0x2e + lna_gain arm is RT2860/RT2870-class silicon [SRC rt2800lib.c:5728-5741].
...
def reset_tuner(t: RT5372Transport, chip: ChipInfo, lna_gain: int) -> None:
    """Re-arm the link tuner to the default VGC [SRC rt2800lib.c:5781-5784
    rt2800_reset_tuner → rt2800_set_vgc]. ``rt2x00link_reset_tuner`` zeroes the cached
    vgc_level before this, so the write always happens (the non-rx-chain arm)."""
    t.bbp_write(66, get_default_vgc(chip, lna_gain))


def link_tuner(t: RT5372Transport, chip: ChipInfo, lna_gain: int, rssi: int,
               vgc_level: int) -> int:
    """Periodic ~1 Hz AGC step [SRC rt2800lib.c:5787-5830 rt2800_link_tuner].

    Returns the new vgc_level. The default (non-RT3572/3593/3883/5592) arm applies to
    this card: bump VGC by 0x10 when RSSI beats -80 dBm. Writes BBP66 only when the
    level changes (``rt2800_set_vgc`` guard).

    Not exercised in monitor mode (``intf_sta_count == 0`` ⇒ never scheduled), so it is
    a no-op in the airmon capture; ported for a future managed-mode port.
    """
