"""RTL8821AU (vendor/DKMS port) — live WEP ARP-replay / IV-generation harness (M6).
...
fallback -> PTW crack) against a WEP AP. The replay injects via the same M6
``inject_frame`` path the deauth harness exercises — so if deauth injection works, this
works; a TX-path fix lands in the shared path and propagates here. The replayed ARP is
already WEP-encrypted, so it is injected raw (descriptor SEC_TYPE = 0).

Needs the WEP test router (channel 6 in the lab; key "abcde"). This TRANSMITS: fake-auth
associates a forged STA with the target AP, then ARP replay floods it. Point it only at
a WEP AP you own. ``--dry-run`` brings up + tunes + constructs the campaign but starts
...
Usage (card plugged in; on Linux unbind the kernel driver, on Windows Zadig/WinUSB):
    ...
    .venv\\Scripts\\python.exe scripts\\rtl8821au_dkms\\wep_replay_hw.py \\
...
def main() -> int:
    ap = argparse.ArgumentParser(description="RTL8821AU DKMS WEP ARP-replay harness (M6)")
