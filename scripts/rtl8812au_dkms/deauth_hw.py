"""RTL8812AU (vendor/DKMS port) — live targeted-deauth injection harness (M6).

Drives the DKMS driver directly: brings the card up, tunes to the target AP's channel,
and injects a burst of the classic bidirectional deauth (spoof-AP -> client and
spoof-client -> AP) via ``driver.inject_frame`` — the M6 TX path. This is the LIVE gate
for TX: morrownr's cold-boot captures contain no successful card TX (that DKMS build
never injected — aireplay put 0 frames on the bus), so there is no byte-for-byte
reference. The proof is behavioural and on this antenna.

THE SIGNAL when the phone shows nothing: a successful deauth knocks a WPA client off, so
it reconnects and emits the 4-way handshake. This harness watches the RX stream for the
target client's EAPOL frames (and the ToDS/FromDS direction) — client EAPOL appearing
after a burst == the deauth landed, even with no visible indication on the device itself.

TX power is the BB-default (per-rate EFUSE TX power is a separate deferred milestone), so
a *nearby* target is the reliable test; a distant client may not be reached yet.
...
Usage (card plugged in; Windows Zadig/WinUSB, or Linux after unbinding the kernel driver):
    ...
    .venv\\Scripts\\python.exe scripts\\rtl8812au_dkms\\deauth_hw.py \\
        --bssid AA:BB:CC:DD:EE:FF --client 00:11:22:33:44:55 --channel 1 --dry-run
    # LIVE — actually injects the burst (omit --dry-run):
    .venv\\Scripts\\python.exe scripts\\rtl8812au_dkms\\deauth_hw.py \\
        --bssid AA:BB:CC:DD:EE:FF --client 00:11:22:33:44:55 --channel 1 --count 20
...
async def run(args) -> int:
    ...
    if not await driver.connect(progress):   # connect() claims the interface + brings up
...
def main() -> int:
    ap = argparse.ArgumentParser(description="RTL8812AU DKMS targeted-deauth harness (M6)")
