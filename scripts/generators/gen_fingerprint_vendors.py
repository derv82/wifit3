#!/usr/bin/env python3
"""Regenerate ``src/wifit3/wlan/fingerprinting/vendors.py`` from Wireshark's ``manuf`` feed.

Client fingerprinting's low-confidence tier (wlan/fingerprinting/client.py) names a client's vendor
...
def main(source: str | None = None) -> None:
    ...
    header = (
        ...
        "(wlan/fingerprinting/client.py): every registered vendor, since naming who made a device --\n"
        ...
        "between several organizations). fingerprinting/client.py tries the longest prefix first.\n\n"
