"""RT5372 (Panda PAU05/PAU06) — live targeted-deauth injection harness (TX validation).

Drives the clean-room ``RT5372Driver`` DIRECTLY (like ``beacon_watch.py``), bypassing the
...
-> bulk-OUT). This is the live gate for TX: does the card actually emit a deauth that drops
a client, and do we then capture the reconnect's 4-way handshake (EAPOL).
...
Usage (card plugged in + Zadig/WinUSB-bound to 148f:5372):
    # SAFE preview — brings up + tunes + builds frames, transmits NOTHING:
    uv run python scripts/rt5372/deauth_hw.py \\
...
from wifit3.chips.rt5372 import tx
...
def main() -> int:
    ap = argparse.ArgumentParser(description="RT5372 (PAU05/PAU06) targeted-deauth TX harness")
