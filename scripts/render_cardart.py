...
never widens the card (keeps every card's on-page footprint uniform).
    ...
    uv run python scripts/render_cardart.py                # all cards -> assets/cardart/
    uv run python scripts/render_cardart.py card-pau06     # one card, by stem or path
    uv run python scripts/render_cardart.py --scale 24     # bigger square-pixel size
    uv run python scripts/render_cardart.py --stack stack-mt7921au card-awus036axml card-pau0f
"""
