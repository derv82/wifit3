...
ONE monotonic walk, ONE cursor, fail-closed. Modelled on ``scripts/rt5372/verify_pcap.py``
(the 2T2R sibling) and ``scripts/rt3070/verify_pcap.py``: the replay is at the
...
RULES (do not violate — this is the whole point of the gate):
  ...
  * The cursor only advances by reproducing the wire or by an explicit named waiver.
    ...
    uv run python scripts/verify_pcap.py rt5370 [capture-1|capture-2|capture-3]
"""
