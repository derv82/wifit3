"""Acceptance gate: replay-diff the clean-room rt5372 (RT5392) port against its cold-boot capture.

ONE monotonic walk, ONE cursor, fail-closed. Modelled on ``scripts/chips/rt3070/verify_pcap.py``
(itself the rtl8188eus_dkms gold standard adapted to the rt2x00 USB family): the replay is at
the ``ctrl_transfer`` layer (``rt2x00_pcap_replay.ReplayDevice``) and the REAL
``chips/rt5372`` transport drives it, so every helper (regbusy poll, write_multi chunking,
...
~1 Hz BBP66 AGC work.
    ...
    uv run python scripts/porting/verify_pcap.py rt5372 [capture-1|capture-2|capture-3]
"""
...
_IMPORT_ERR = None
try:
    from wifit3.chips.rt5372.transport import RT5372Transport  # noqa: E402
    from wifit3.chips.rt5372 import (  # noqa: E402
...
def _walk_init(w: Walk, out: dict) -> None:
    ...
    WIRE ORDER (rt2x00 family, same kernel driver as rt3070): probe → EFUSE → gpio →
    ...
    --- M2+ frontier: enable_radio / init_registers / BBP / RFCSR / channel ---
    """
    ...
    out["drv"] = w.run(lambda t: rfcsr.init_rfcsr_5392(t, chip, ev), "init-rfcsr")  # RFCSR (no cal)
...
def run(cap: str | None = None) -> int:
    ...
    if _IMPORT_ERR is not None:
        print(f"\nrt5372 driver not scaffolded yet ({_IMPORT_ERR}).")
        first = ops[0] if ops else None
        if first:
            print(f"FRONTIER: op 0 = {rp.ReplayDevice._fmt(first)}")
        print("  ^ start at M1 (probe + EFUSE). Build chips/rt5372/{transport,eeprom,...}.py "
              "from the kernel C; see chips/rt5372/RT5372.md. Re-run after each milestone.")
        return 1
