"""Passive driver-path RX probe — isolates the deauth_hw "ap_beacons=0" failure.

deauth_hw drives the full driver (connect -> RxReaderThread -> enable_monitor -> watchdog) and saw
0 RX frames, while cck_diag (a synchronous bulk_in loop) hears beacons fine. This runs the SAME
driver RX path deauth_hw uses — connect + the reader thread + the 2 s watchdog — but injects NOTHING
(passive, agent-safe), and tallies frames for `--listen` seconds. It tells us whether the failure is
(a) the driver/reader RX path itself, or (b) the concurrent TX (inject) in deauth_hw starving RX.

Run: uv run python scripts/rtl8822bu_dkms/driver_rx_probe.py --channel 1 --listen 15
"""
...
async def run(args) -> int:
    ...
    if tally["frames"] == 0:
        ...
        print("  => driver RX path works passively -> deauth_hw's 0-frames is TX/inject contention.")
    return 0
