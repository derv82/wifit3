...
Usage:
    uv run python scripts/mt76x0u/capture_our_hops.py --usbpcap USBPcap2
...
After running, slice and compare:
    uv run python scripts/pcap_slicer.py \
        scripts/mt76x0u/captures_ours/ours-*.main.log \
        scripts/mt76x0u/captures_ours/ours-*.pcap
"""
...
def write_main_log(log_path: Path, events: list[tuple[float, str]]) -> None:
    ...
    Format (per scripts/pcap_slicer.py:8):
...
async def main_async(args):
    ...
    print()
    print("Next steps:")
    print(f"  uv run python scripts/pcap_slicer.py {log_path} {pcap_path}")
